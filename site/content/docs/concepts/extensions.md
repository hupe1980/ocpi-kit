...
## Why it matters in a hub

A hub sits between many parties. If a CPO sends a Location with a vendor field and the hub decodes
it into a strict struct and re-encodes it, the field is gone before it reaches the eMSP that needed
it — silently, and nothing fails. A crate that cannot represent the unknown cannot forward it.
